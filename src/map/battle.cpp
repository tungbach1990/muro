...
struct Damage battle_calc_attack(int attack_type,struct block_list *bl,struct block_list *target,uint16 skill_id,uint16 skill_lv,int flag)
{
	...
	if (sd && tsd)
	{
		...
		max_damage = cap_value(max_damage,999,899999999);
	...
	if (sd && tmd)
	{
		...
		max_damage = cap_value(max_damage,9999,899999999);
		max_damage = (int64)max_damage * (100 + sd->bonus.max_damage_exceed - tmd->custom.max_damage_pen_exceed) / 100 ;
		d.damage = cap_value(d.damage,INT_MIN,max_damage);
		if (skill_id) 
		{
			if ( rand()%100 < cap_value(sd->bonus.max_rate - tmd->custom.max_pen_rate,1,1000))
				d.damage = max_damage;
			if ( rand()%100 < tmd->custom.max_eva - sd->bonus.max_pen_eva)
				d.damage = 0;
			if ( rand()%100 < tmd->custom.max_block - sd->bonus.max_pen_block)
				d.damage = 1;
		}
	}
	if (md && tsd)
	{
		...
		max_damage = cap_value(max_damage,999,899999999);
		max_damage = (int64)max_damage * (100 + md->custom.max_damage_exceed - tsd->bonus.max_damage_pen_exceed) / 100 ;
		//d.damage = cap_value(d.damage,INT_MIN,max_damage);
		if (skill_id) 
		{
			if ( rand()%100 < cap_value(md->custom.max_rate - tsd->bonus.max_pen_rate,1,1000))
				d.damage = max_damage;
			if ( rand()%100 < tsd->bonus.max_eva - md->custom.max_pen_eva)
				d.damage = 0;
			if ( rand()%100 < tsd->bonus.max_block - md->custom.max_pen_block)
				d.damage = 1;
		}
