...
int status_calc_mob_(struct mob_data* md, uint8 opt)
{
	...
	if (opt&SCO_FIRST) { // Set basic level on respawn.
		...
		md->custom.max_damage = md->db->custom.max_damage;
		md->custom.max_rate = md->db->custom.max_rate;
		md->custom.max_eva = md->db->custom.max_eva;
		md->custom.max_block = md->db->custom.max_block;
		md->custom.max_pen_eva = md->db->custom.max_pen_eva;
		md->custom.max_pen_block = md->db->custom.max_pen_block;
		md->custom.max_pen_rate = md->db->custom.max_pen_rate;
		md->custom.max_damage_exceed = md->db->custom.max_damage_exceed;
		md->custom.max_damage_pen_exceed = md->db->custom.max_damage_pen_exceed;
	}
...
int status_calc_pc_sub(map_session_data* sd, uint8 opt)
{
	...
	memset(&sd->custom, 0, sizeof(sd->custom));
	// Autobonus
...
//		if (util::vector_exists(status_get_race2(bl), RC2_GVG) && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
		...
		case SC_ENDURE:
			...
			if( !(flag&SCSTART_NOAVOID) && (bl->type&(BL_PC|BL_MOB|BL_MER)) && !map_flag_gvg2(bl->m) && !map_getmapflag(bl->m, MF_BATTLEGROUND) && !val4 ) {
				block_list *tbl;
				map_session_data* tsd;
				if( sd ) {
					int i;
					for( i = 0; i < MAX_DEVOTION; i++ ) {
						if( sd->devotion[i] && (tbl = map_id2bl(sd->devotion[i])) )
							status_change_start(src,tbl, type, 10000, val1, val2, val3, val4, tick, SCSTART_NOAVOID);
					}
				} else if (md) {
					int i;
					for( i = 0; i < MAX_DEVOTION; i++ ) {
		...
		case SC_AUTOGUARD: {
			...
			if (!(flag & SCSTART_NOAVOID)) {
				...
				if (bl->type & (BL_PC | BL_MER | BL_MOB)) {
					if (sd) {
						for (i = 0; i < MAX_DEVOTION; i++) {
							if (sd->devotion[i] && (tbl=map_id2bl(sd->devotion[i])))
								status_change_start(src, tbl, type, 10000, val1, val2, 0, 0, tick, SCSTART_NOAVOID);
						}
					} else if (md) {
						for (i = 0; i < MAX_DEVOTION; i++) {
							if (md->devotion[i] && (tbl=map_id2bl(md->devotion[i])))
