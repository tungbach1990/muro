...
//		if (util::vector_exists(status_get_race2(bl), RC2_GVG) && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
		...
		case SC_DEVOTION:
		{
			...
			//		if (d_sc->data[type2]) {
			//			int val1 = d_sc->data[type2]->val1;
			//			int	val2 = d_sc->data[type2]->val2;
			//			int val3 = 0;
			//			int val4 = 0;
			//			if (type2 == SC_DEFENDER) {
			//				val3 = d_sc->data[type2]->val3;
			//			}
		...
		case SC_AUTOGUARD:
			{
				...
				if( bl->type == BL_PC ) { // Clear Status from others
					...
					for( i = 0; i < MAX_DEVOTION; i++ ) {
						...
						if( sd->devotion[i] && (tbl = map_id2bl(sd->devotion[i])) && status_get_sc(tbl)->data[type] )
							status_change_end(tbl, type);
					}
				}else if( bl->type == BL_MOB ) { // Clear Status from others
					int i;
					for( i = 0; i < MAX_DEVOTION; i++ ) {
						if(md->devotion[i] && (tbl = map_id2bl(md->devotion[i])) && status_get_sc(tbl)->data[type] )
							status_change_end(tbl, type);
